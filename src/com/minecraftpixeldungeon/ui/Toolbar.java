 ...
 */
package com.minecraftpixeldungeon.ui;
...
import com.minecraftpixeldungeon.windows.WndCatalogus;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		int slots = MinecraftPixelDungeon.quickSlots();
		...
		for(int i = 0; i <= 3; i++) {
			...
			if (slots == 4 && width < 150){
				if (width < 139){
					if ((MinecraftPixelDungeon.flipToolbar() && i == 3) ||
							(!MinecraftPixelDungeon.flipToolbar() && i == 0)) {
					...
					if (i == 0 && !MinecraftPixelDungeon.flipToolbar() ||
						i == 3 && MinecraftPixelDungeon.flipToolbar()){
						btnQuick[i].border(0, 2);
						btnQuick[i].frame(106, 0, 19, 24);
					} else if (i == 0 && MinecraftPixelDungeon.flipToolbar() ||
							i == 3 && !MinecraftPixelDungeon.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(MinecraftPixelDungeon.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
		...
		if (MinecraftPixelDungeon.flipToolbar()) {
