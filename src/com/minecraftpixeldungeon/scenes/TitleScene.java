...
import com.minecraftpixeldungeon.effects.Fireball;
...
import com.minecraftpixeldungeon.ui.ChangesButton;
import com.minecraftpixeldungeon.ui.ExitButton;
import com.minecraftpixeldungeon.ui.PrefsButton;
import com.minecraftpixeldungeon.ui.Window;
import com.minecraftpixeldungeon.windows.WndHardNotification;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		float height = title.height +
				(MinecraftPixelDungeon.landscape() ? DashboardItem.SIZE : DashboardItem.SIZE * 2);
		...
		DashboardItem btnBadges = new DashboardItem( TXT_BADGES, 3 ) {
			...
			protected void onClick() {
				MinecraftPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		add(btnBadges);
		...
		DashboardItem btnAbout = new DashboardItem( TXT_ABOUT, 1 ) {
			@Override
			protected void onClick() {
				MinecraftPixelDungeon.switchNoFade( AboutScene.class );
			}
		};
		add( btnAbout );
		...
		DashboardItem btnPlay = new DashboardItem( TXT_PLAY, 0 ) {
			@Override
			protected void onClick() {
				MinecraftPixelDungeon.switchNoFade( StartScene.class );
			}
		};
		add( btnPlay );
		...
		DashboardItem btnHighscores = new DashboardItem( TXT_HIGHSCORES, 2 ) {
			@Override
			protected void onClick() {
				MinecraftPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		add( btnHighscores );

		if (MinecraftPixelDungeon.landscape()) {
		...
		BitmapText shatteredVersion = new BitmapText("Shattered v 0.3.3b", pixelFont);
		shatteredVersion.measure();
		shatteredVersion.hardlight(0xCCCCCC);
		shatteredVersion.x = w - shatteredVersion.width();
		shatteredVersion.y = h - shatteredVersion.height();
		add(shatteredVersion);
		
		BitmapText version = new BitmapText( "v " + Game.version + "", pixelFont);
		version.measure();
		version.hardlight( Window.MPX_COLOR );
		version.x = w - version.width();
		version.y = h - version.height() - shatteredVersion.height();
		add( version );
		
		Button changes = new ChangesButton();
		changes.setPos( w-changes.width(), h - version.height() - shatteredVersion.height() - changes.height());
		add( changes );
		...
		int gameversion = MinecraftPixelDungeon.version();
